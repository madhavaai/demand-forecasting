...
class CleaningPipeline:
    ...
    def parse_timestamps(self, df):
        ...
        local_times = pd.Series(pd.NaT, index=df.index, dtype="object")
        for tz, grp in df.groupby("timezone"):
            idx = grp.index
            local_times.loc[idx] = df.loc[idx, "ts_utc"].dt.tz_convert(tz)
        df["ts_local"] = pd.to_datetime(local_times, utc=False, errors="coerce")
        df["business_date"] = df["ts_local"].dt.date
